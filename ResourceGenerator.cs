...
namespace NFive.PluginManager
{
	...
	public class ResourceGenerator
	{
		...
		/// </value>
		public Guid ManifestVersion { get; set; } = new Guid("44febabe-d386-4d18-afbe-5e627f4af937");
		...
		/// </value>
		public IEnumerable<string> DefaultSeverScripts => new List<string>
		{
		...
		/// </value>
		public IEnumerable<string> DefaultClientScripts => new List<string>
		{
		...
		/// </value>
		public IEnumerable<string> DefaultClientFiles => new List<string>
		{
			"Newtonsoft.Json.dll",
		...
		public string Serialize(DefinitionGraph graph)
		{
			...
			WriteLine(ref output, "resource_manifest_version", this.ManifestVersion.ToString());
			WriteLine(ref output);

			WriteLine(ref output, "server_scripts {");
			WriteLine(ref output, "\t-- NFive");
			foreach (var file in this.DefaultSeverScripts) WriteLine(ref output, $"\t'{file}',");
			WriteLine(ref output, "}");
			WriteLine(ref output);
			...
			foreach (var plugin in graph.Definitions.Where(d => d.Client?.Include?.Count > 0 || d.Client?.Main?.Count > 0))
			{
				...
				if (plugin.Client?.Include != null) foreach (var file in plugin.Client.Include) WriteLine(ref output, $"\t'{Path.Combine(Program.PluginPath, plugin.Name.Vendor, plugin.Name.Project, file.Replace(Path.AltDirectorySeparatorChar, Path.DirectorySeparatorChar))}.net.dll',");
				if (plugin.Client?.Main != null) foreach (var file in plugin.Client.Main) WriteLine(ref output, $"\t'{Path.Combine(Program.PluginPath, plugin.Name.Vendor, plugin.Name.Project, file.Replace(Path.AltDirectorySeparatorChar, Path.DirectorySeparatorChar))}.net.dll',");
				WriteLine(ref output);
			}
			WriteLine(ref output, "\t-- NFive");
			foreach (var file in this.DefaultClientScripts) WriteLine(ref output, $"\t'{file}',");
			WriteLine(ref output, "}");
			WriteLine(ref output);

			WriteLine(ref output, "files {");
			WriteLine(ref output, "\t-- NFive");
			foreach (var file in this.DefaultClientFiles) WriteLine(ref output, $"\t'{file}',");
			foreach (var plugin in graph.Definitions.Where(d => d.Client?.Files?.Count > 0))
			{
				WriteLine(ref output);
				WriteLine(ref output, $"\t-- {plugin.Name}@{plugin.Version}");
				foreach (var file in plugin.Client.Files) WriteLine(ref output, $"\t'{Path.Combine(Program.PluginPath, plugin.Name.Vendor, plugin.Name.Project, file.Replace(Path.AltDirectorySeparatorChar, Path.DirectorySeparatorChar))}',");
			}
			WriteLine(ref output, "}");
