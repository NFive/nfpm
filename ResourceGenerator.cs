	...
	public static class ResourceGenerator
	{
		...
		public static ResourceString Serialize(DefinitionGraph graph)
		{
			...
			foreach (var plugin in graph.Plugins.Where(d => d.Client?.Include?.Count > 0 || d.Client?.Main?.Count > 0))
			{
				...
				if (plugin.Client?.Main != null) foreach (var file in plugin.Client.Main) WriteLine(ref output, $"\t'{Path.Combine(ConfigurationManager.PluginPath, plugin.Name.Vendor, plugin.Name.Project, file).Replace(Path.DirectorySeparatorChar, '/')}.net.dll',");
				WriteLine(ref output);
			}
			WriteLine(ref output, "\t-- NFive");
			...
			foreach (var plugin in graph.Plugins.Where(d => d.Client?.Files?.Count > 0 || d.Client?.Overlays?.Count > 0))
			{
				WriteLine(ref output);
				WriteLine(ref output, $"\t-- {plugin.Name}@{plugin.Version}");
				foreach (var file in plugin.Client.Files) WriteLine(ref output, $"\t'{Path.Combine(ConfigurationManager.PluginPath, plugin.Name.Vendor, plugin.Name.Project, file).Replace(Path.DirectorySeparatorChar, '/')}',");
			}
			WriteLine(ref output, "}");
